...
export function formatComponentTree(nodes: SvelteNode[]): string {
  const output: string[] = [
  ...
  const children = new Map<number, SvelteNode[]>();
  ...
  function walk(node: SvelteNode, depth: number) {
...
export function formatComponentDetails(nodes: SvelteNode[], id: string): string {
  const match = nodes.find((n) => String(n.id) === id);
  if (!match) return `Component ${id} not found. Run 'svelte tree' to get fresh IDs.`;
...
function collectSvelteNodes(): SvelteNode[] {
